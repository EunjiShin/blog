...
@SqlGroup({
    @Sql(value = "/sql/user-service-test-data.sql", executionPhase = ExecutionPhase.BEFORE_TEST_METHOD), // 테스트 시작 전 데이터 넣는 sql
    @Sql(value = "/sql/delete-all-data.sql", executionPhase = ExecutionPhase.AFTER_TEST_METHOD) // 테스트 종료 후 모든 테스트 데이터를 지우는 sql
})
public class UserServiceTest {
    ...
    void userCreateDto_를_이용하여_유저를_생성할_수_있다() {
        ...
        UserCreateDto userCreateDto = UserCreateDto.builder()
            ...
            .nickname("kok202-k")
            .build();
        // FIXME: 이메일 발송하는 mailSender mock
